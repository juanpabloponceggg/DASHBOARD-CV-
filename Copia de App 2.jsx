  ...
  useEffect(() => {
    const loadEjecutivos = async () => {
      try {
        ...
        setEjecutivosList((data || []).map(e => e.nombre));
      } catch (err) {
        ...
        >
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} style={{ textAlign: "center", padding: 40, color: COLORS.textLight, fontSize: 14 }}>
...
function GestionUsuarios() {
  ...
  // Load users and ejecutivos from Supabase
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        console.error("Error loading users/ejecutivos:", err);
...
function ResumenNomina() {
  ...
  const tableData = useMemo(() => {
    return nominaEjecutivos.map((ej) => {
      // Calculate real sales: sum of amounts where ejecutivo_id matches, estatus = "Dispersión", and producto = "Crédito de nómina"
      const real = clients
        .filter(
          (c) =>
            c.ejecutivo_id === ej.id &&
...
function ResumenMotos() {
  ...
  const tableData = useMemo(() => {
    return motosEjecutivos.map((ej) => {
      // Count units: where ejecutivo_id matches, estatus = "Dispersión", and producto != "Crédito de nómina"
...
function ExecutiveTable({ title, subtitle, data, isMoney, accentColor, icon, type, onUpdateMeta, onToggleActivo }) {
...
function CatalogoEjecutivos() {
  ...
  const { nominaEjecutivos: nomina, motosEjecutivos: motos, loading, updateMeta, toggleActivo, copyFromPreviousMonth } = useExecutives({ mes, anio });
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 11, color: COLORS.primary, fontWeight: 600, textAlign: "center", marginTop: 10, marginBottom: 20 }}>
          Haz clic en cualquier meta para editarla. Agrega o desactiva ejecutivos según sea necesario.
        </p>
      </div>
    </div>
  );
...
function PortalEjecutivo({ perfil }) {
  ...
  const [mes, setMes] = useState(now.getMonth() + 1);
  const [anio, setAnio] = useState(now.getFullYear());
  // Buscar nombre exacto del ejecutivo desde tabla ejecutivos (es el que se usa en clientes)
  const [ejecutivoNombre, setEjecutivoNombre] = useState(null);
  const [nombreCargado, setNombreCargado] = useState(false);

  useEffect(() => {
    const loadNombre = async () => {
      if (perfil?.ejecutivo_id) {
        const { data } = await supabase
          .from("ejecutivos")
          .select("nombre")
          .eq("id", perfil.ejecutivo_id)
          .single();
        if (data) {
          setEjecutivoNombre(data.nombre);
          setNombreCargado(true);
          return;
        }
      }
      // Fallback: usar nombre_display del perfil
      setEjecutivoNombre(perfil?.nombre_display || null);
      setNombreCargado(true);
    };
    loadNombre();
  }, [perfil?.ejecutivo_id, perfil?.nombre_display]);

  const { clients, loading: clientsLoading, error, addClient, updateEstatus, deleteClient, refetch } = useClients({
    mes, anio,
    ejecutivoId: perfil?.ejecutivo_id,
    ejecutivoNombre: nombreCargado ? ejecutivoNombre : null,
    isAdmin: false,
  });
  const loading = !nombreCargado || clientsLoading;
  ...
  useEffect(() => {
    const loadClients = async () => {
      ...
      try {
        ...
        console.error("Error loading clients for export:", err);
        setAllClients([]);
      } finally {
      ...
      <main style={{
        ...
        {isEjecutivo ? (
          ...
          <>
            ...
            {!["clientes", "nomina", "motos", "catalogo", "usuarios", "export"].includes(activeScreen) && (
